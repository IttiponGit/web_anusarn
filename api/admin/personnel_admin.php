...
function normalize_personnel_payload(array $input): array
{
    ...
    return [
        ...
        'subject_group' => trim((string)($input['subject_group'] ?? ($input['department'] ?? ''))),
...
{
    $requiredFields = ['name', 'position', 'group_name', 'image', 'status'];
...
try {
    if ($method === 'GET') {
        ...
        $baseSql = 'SELECT `id`, `name`, `position`, `department`, `department` AS `subject_group`, `group_name`, `image`, `display_order`, `status`, `created_at`, `updated_at` FROM `personnel`';
    ...
    if ($method === 'POST') {
        ...
        $insertStmt->execute([
            ...
            ':department' => $payload['subject_group'],
